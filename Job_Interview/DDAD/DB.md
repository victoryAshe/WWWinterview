...
### 정규화 1차 2차 3차 BCNF
* 정규화
    - 한 릴레이션 내에서 최소한의 데이터 중복과 최대한의 데이터 유연성,일관성을 위해 데이터를 분리하는 과정 
     => 데이터의 삽입/갱신/삭제 이상현상을 방지하고 논리적이고 직관적인 테이블 구성을 가지게 함
* 1차 정규화
    - 모든 속성의 도메인이 원자값으로만 구성되어야 한다. 
    즉, 로우마다 컬럼 값이 1개씩만 존재해야한다.
  
* 2차 정규화
    - 복합키의 경우, 완전 함수적 종속성을 가져야 한다. 
    즉, 특정 PK에 의한 함수적 종속이 존재하지 않아야 한다.
 
* 3차 정규화 
    - 이행적 함수적 종속성을 만족해야 한다. 
        즉, 기본키에 의해서만 함수적 종속성을 만족해야 한다.
  
* BCNF
    - 3차 정규형을 만족하면서 모든 결정자가 후보키 집합에 속해야 한다. 
     즉, 결정자 중 후보키가 아닌 것들은 제거해야 한다.
...
* 트랜잭션(Transaction) 이란
    ...
    * 
        * Commit 연산
            * 트랜잭션에 대한 작업이 성공적으로 끝나 데이터베이스가 다시 일관된 상태에 있을 때, 완료된 것을 트랜잭션 관리자에게 알려주는 연산
        * Rollback 연산
...
* Isolation Level 의 필요성
    ...
    * Lock
       트랜잭션 처리의 순차성을 보장받기 위한 방법 
     - 종류 
   1. Shared Lock(공유 락) 
      테이블의 데이터를 읽을 때 사용하는 락 
     - 공유 락간의 동시 접근 가능 
     - 공유 락이 설정된 데이터에 배타적 락 사용 X 
   2. Exclusive Lock(배타 락)
     테이블의 데이터를 변경할 때 사용하는 락. 
    - 트랜잭션 완료때까지 유지되고 해제될 때까지 다른 트랜잭션(읽기 포함)은 해당 리소스에 접근 불가 
    - 트랜잭션이 수행되고 있는 데이터에 접근하여 동시 Lock 설정 X
...
* Isolation Level 의 종류
    ...
    2. Read Committed (레벨 1)
        ...
        * 단, Commit이 이루어진 트랜잭션만 조회할 수 있다.
          (트랜잭션의 전체 실행까지 안기다려도 됨)
        * => 어떤 사용자가 A라는 데이터를 B라는 데이터로 변경하는 동안 다른 사용자는 해당 데이터에 접근할 수 없다.
        * - Oracle, SQL Server의 Default Isolation Level
    ...
    3. Repeatable Read (레벨 2)
        ...
        * - Non-Repeatable Read 부정합 발생 방지
        * - Phantom Read 발생
    4. Serializable (레벨 3)
        * 트랜잭션이 완료될 때까지 SELECT 문장이 사용하는 모든 데이터에 Shared Lock이 걸리는 Level
        * 엄격한 읽기 일관성 모드를 제공한다.
        * => 다른 사용자는 그 영역에 해당되는 데이터에 대한 수정 및 입력이 불가능하다.
       (2,3의 차이는 데이터에 대한 입력의 가능 여부)
...
* 낮은 단계의 Isolation Level 이용시 발생하는 현상  
    <img src="https://raw.githubusercontent.com/WeareSoft/tech-interview/master/contents/images/isolation-level.png" width="70%" height="70%">
    * Dirty Read
